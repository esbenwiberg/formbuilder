...
import { IJsonMemberSchema, IJsonPropertySchema } from "./JsonPropertySchema";
...
const parseSingleProperty = (jsonPropertySchema: IJsonPropertySchema, formSchema: IFormSchema<DynamicListFormItem>, nested?: boolean) : void => {
    let name: string = getName(jsonPropertySchema.Name);
    let type = mappings[jsonPropertySchema.Type];

    let isList = jsonPropertySchema.Type === "List";
    ...
    let options = propertyOptions(type, name);
    ...
    if (type == PropertyType.Array) {
        let dynamicArrayConfig = (options as IFormItemPropertyOptions<DynamicListFormItem, IDynamicArrayFieldConfig>).config;
    ...
    if (type == PropertyType.PredefinedArray) {
        let arrayOptions = options as IFormItemPropertyOptions<DynamicListFormItem, IDynamicPredefinedArrayFieldConfig>;
    ...
    if (type == PropertyType.FormItem) {
        if (isList) {
            let listFormSchema = { options: { properties: { }, validation: { validationRules: { } } } } as IFormSchema<DynamicListFormItem>;
            let listSchema = jsonPropertySchema.ElementType;
            
            listSchema.Properties.forEach(_ => parseSingleProperty(_, listFormSchema, true));
            let listPropName = jsonPropertySchema.Name;

            // validation
            let validationRules = {} as  any;
...
export const PluginSchemaParser = (jsonSchema: any) : IFormSchema<DynamicListFormItem> | undefined => {
    ...
    let formSchema = { options: { properties: { }, validation: { validationRules: { } } } } as IFormSchema<DynamicListFormItem>;

    let memberSchema: IJsonMemberSchema = jsonSchema.Request as IJsonMemberSchema;

    let map = mappings[memberSchema.Type];
